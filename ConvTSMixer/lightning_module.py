...
from .module import ConvTSMixerModel


class ConvTSMixerLightningModule(pl.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a
    ``ConvTSMixerModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``ConvTSMixerModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model
        ``ConvTSMixerModel`` to be trained.
    lr
    ...
    def __init__(
        ...
        self.model = ConvTSMixerModel(**model_kwargs)
        self.lr = lr
