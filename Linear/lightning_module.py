...
class LinearLightningModule(pl.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a
    ``LinearModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``LinearModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model
        ``LinearModel`` to be trained.
    loss
    ...
    def __init__(
        ...
        self.model = LinearModel(**model_kwargs)
        self.loss = loss
        self.lr = lr
